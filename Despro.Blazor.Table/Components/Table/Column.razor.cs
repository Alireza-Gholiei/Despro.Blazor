...
namespace Despro.Blazor.Table.Components.Table
{
    public class ColumnBase<TItem> : BaseComponent, IColumn<TItem>
    {
        ...
        public string Title
        {
            get
            {
                string title = _title ?? Property.GetPropertyMemberInfo()?.Name;
        ...
        protected override void OnParametersSet()
        {
            try
            {
                ...
                if (Property != null)
                {
                    MemberInfo memberInfo = Property.GetPropertyMemberInfo();

                    Type underlyingType = memberInfo.GetMemberUnderlyingType();
        ...
        public async Task GroupByMeAsync()
        {
            try
            {
                if (Groupable)
                {
                    ...
                    {
                        foreach (IColumn<TItem> column in Table.Columns.Where(e => e.GroupBy))
                        {
