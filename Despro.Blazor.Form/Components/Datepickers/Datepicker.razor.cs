    ...
    public partial class Datepicker<TValue> : BaseComponent
    {
        ...
        private DateTimeOffset FirstYearInTen(DateTime dt)
        {
            try
            {
                ...
                string yearSt = persianCalendar.GetYear(dt).ToString();

                int year = int.Parse(yearSt.Substring(yearSt.Length - 1, 1));
        ...
        private List<DateTimeOffset> GetDates()
        {
            try
            {
                ...
                DateTime firstDayOfMonth = _currentDate.Date.AddDays(1 - persianCalendar.GetDayOfMonth(_currentDate.DateTime));
                DateTimeOffset firstDate = FirstDateInWeek(firstDayOfMonth);
        ...
        private List<DateTimeOffset> GetMounts()
        {
            try
            {
                ...
                DateTimeOffset firstYear = _currentDate;

                List<DateTimeOffset> mounts = new();

                firstYear = firstYear.AddMonths(1 - persianCalendar.GetMonth(firstYear.DateTime));

                int daysOfMonth = persianCalendar.GetDayOfMonth(firstYear.DateTime);
                int daysInMonthCur = persianCalendar.GetDaysInMonth(firstYear.Year, firstYear.Month);
                ...
                {
                    int daysInMonth = persianCalendar.GetDaysInMonth(persianCalendar.GetYear(firstYear.DateTime), persianCalendar.GetMonth(firstYear.DateTime));
        ...
        private List<DateTimeOffset> GetYears()
        {
            try
            {
                ...
                DateTimeOffset firstYear = FirstYearInTen(_currentDate.Date);
